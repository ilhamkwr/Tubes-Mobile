...
class MainPageState extends State<MainPage> {
  ...
  Future <List<Jadwal>> _getJadwal() async {
    var jadwalRespon = await http.post('http://157.230.245.30/sensasiq/api/jadwal/jadwal', body: {
      "nim": widget.nimnya
    });
  ...
  Future <List<Riwayat>> _getRiwayat() async {
    var riwayatRespon = await http.post('http://157.230.245.30/sensasiq/api/absen/absen', body: {
      "nim": widget.nimnya
    });
    ...
    Future _scanQR() async {
      try {
        ...
        this.result = qrResult;
          this.indexMenu = 1;
          final response = await http.post("http://157.230.245.30/sensasiq/api/qr/cocok", body: {
           "qr": result,
          });
          ...
          if((datauser['error']) || (datauser['qr'][0]['qr']!=result)){
            ...
            final hasil = await http.post("http://157.230.245.30/sensasiq/api/absen/add", body: {
